...
int counts = 0;  // used to control display output rate
...
DigitalOut MPU6050_Success(PB_5);
DigitalOut MPU6050_Error(PB_4);
...
class MPU6050 {
    ...
    void readBytes(uint8_t address, uint8_t subAddress, uint8_t counts, uint8_t * dest)
    {
        char data[14];
        char data_write[1];
        data_write[0] = subAddress;
        i2c.write(address, data_write, 1, 1); // no stop
        i2c.read(address, data, counts, 0); 
